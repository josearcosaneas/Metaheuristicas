...
void BL::setvectorsolucion(vector<int> _sol){
    vectorsolucion=_sol;
}

vector <int> BL::getvectorsolucion(){
...
void BL::ejecuta(int veces){
    ...
    while (eva<veces && mejora){
        ...
        t4=clock();
        //cout << "Porcentaje: " << (float)eva/(float)veces*100.0 << "% ---- tiempo estimado: "<< ((t4-t3)/CLOCKS_PER_SEC*(100-(eva/veces)*100))/60 << " minutos"  << endl;
    }
    t2=clock();
    tiempoejecucion+=t2-t1;
